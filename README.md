...
Telegram → Anki vocabulary bot. Send English words; receive Anki cards with Sinhala + English meanings, two example sentences, and MAI-Voice-2 TTS audio — upserted into a `Daily` deck via AnkiConnect and synced to a self-hosted Anki sync server.
...
The workflow builds a Docker image, pushes it to GHCR, then SSHes to the VPS and runs `docker compose pull && docker compose up -d`. Secrets (`VPS_HOST`, `VPS_USER`, `VPS_SSH_KEY`) must be set in the repository's GitHub Actions settings.
...
After that first login the profile is persisted on a Docker volume and the bot runs unattended.
